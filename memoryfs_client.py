...
class DiskBlocks():
  def __init__(self, args):
    ...
    if args.total_num_servers:
      self.server_number = args.total_num_servers
    else:
      print('Must specify valid server number')
      quit()
    self.load = []
...
## Blocks are padded with zeroes up to BLOCK_SIZE
  ...
  def xor_function(self,x,y):
    return bytearray(a^b for(a,b) in zip(x,y))

  #allocate server to be a parity server every N blocks where N is number of server.
  #The chart to introduce RAID 5 block allocation is written in the report.
  ...
  def Put(self, virtual_block_number, block_data):
    ...
    if virtual_block_number in range(0,TOTAL_NUM_BLOCKS):
      ...
      physical_block,server_id,parity_server_id = self.virtual_to_physical(virtual_block_number)
      putdata = bytearray(block_data.ljust(BLOCK_SIZE,b'\x00'))
      #calculate the parity block content as well
      # Write block
      ...
      try:
        self.block_server[server_id].Put(physical_block,putdata)
        self.load[server_id]+=1
      except socket.error:
        logging.debug('server failed')

      try:
        self.block_server[parity_server_id].Put(physical_block,new_parity)
        self.load[parity_server_id]+=1
      except socket.error:
        logging.debug('server failed')
        #print('server failed')
      ...
      #ret = self.block_server[server_id].Put(physical_block,putdata)
      #ret2 = self.block_server[parity_server_id].Put(physical_block,new_parity)
      #if ret == -1:
    ...
    logging.error('Put: Block out of range: ' + str(physical_block))
    quit()
...
## Equivalent to the textbook's BLOCK_NUMBER_TO_BLOCK(b)
  ...
  def ServerGet(self,server_id,block_number):
    if block_number in range(0, math.floor(TOTAL_NUM_BLOCKS / (self.server_number - 1))):
      try:
        logging.debug ('ServerGet: ' + 'Server: ' + str(server_id) + '' + str(block_number))
...
## Copy a block from the cache, or bring a block from server and add to the cache
  ...
  def Get(self, virtual_block_number):
    ...
    if virtual_block_number in range(0,TOTAL_NUM_BLOCKS):
      # is it in the cache?
      if virtual_block_number in self.blockcache:
        logging.debug ('Get: cache hit for ' + str(virtual_block_number))
        return self.blockcache[virtual_block_number]
      else:
        logging.debug ('Get: cache miss for ' + str(virtual_block_number))
        # call Get() method on the server
        physical_block,server_id,parity_server_id = self.virtual_to_physical(virtual_block_number)
        data = self.ServerGet(server_id,physical_block)
        # convert to bytearray
        result = bytearray(data)
        # store to cache
        self.blockcache[virtual_block_number] = result
        return result
